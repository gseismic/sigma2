...
> 历史备份：本文是阶段性系统设计，不作为当前实现依据。当前唯一有效设计见 `docs/design/sigma2-20260704-overview.md`。

## 状态

本文保留应用层和 ML 工作流设计价值，但核心优先级已被 `docs/design/sigma2-20260704-overview.md` 修正。

最新结论：

- sigma2 core 不是 `FeatureSet`。
- sigma2 core 是轻量、可继承、类似 `pyta2.base.rIndicator` 的 `rSignal` / family 子类体系。
- `FeatureSet`、DataFrame batch、online state、minbt adapter 属于应用层或适配层，应消费 core。
- 后续实现顺序应以 `docs/design/sigma2-20260704-overview.md` 为准。
...
sigma2 是面向量化 ML 的信号与特征生成库。应用层可以提供 `FeatureSet` 这类便利门面，但它不是 core 的定义中心。
