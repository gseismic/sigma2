...
def on_bars(self, dt, bars):
    rows = self.features.update_minbt_bars(dt, bars)
```
...
Signal family 由输入类型和输出形态决定。单 symbol 和多 symbol 必须区分，因为它们的状态、输出和 ML 对齐语义不同。
...
```python
from sigma2.adapters.minbt import update_bars, update_books, update_trades
```
