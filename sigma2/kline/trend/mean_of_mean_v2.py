...
def _ma_full_name(indicator: rIndicator) -> str:
...
class rKlineMeanOfMeanV2(rKlineWindowSignal):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        ma_options = dict(ma_kwargs or {})
        forbidden = _FORBIDDEN_MA_KWARGS & ma_options.keys()
        if forbidden:
            raise ValueError(
        ...
        self._inner = ma_cls(
            self.inner_n, buffer_size=0, return_dict=False, **ma_options
        )
        self._outer = ma_cls(
            self.outer_n, buffer_size=0, return_dict=False, **ma_options
        )
        self.ma_type = str(self._inner.name)
        self.ma_kwargs = ma_options
        self.field = field
    ...
    def full_name(self) -> str:
        return (
            f"mean_of_mean_v2({_ma_full_name(self._inner)},"
            f"{_ma_full_name(self._outer)})[{self.field}]"
        )
