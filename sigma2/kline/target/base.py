...
class rKlineEffectSignal(rKlineWindowSignal):
    """把 stateless pyta2 effect 绑定到标准 K 线输入的内部 target 模板。"""

    name = "kline_effect"
    # future target 的输出属于历史 anchor，而不是当前最后一根 K 线；
        ...
        **kwargs: Any,
        ...
        if not _trusted_stateless:
            raise ValueError(
                ...
                "pyta2 effects; use a concrete sigma2.kline.target signal"
            )
    ...
    def forward(self, opens, highs, lows, closes, volumes) -> Any:
        effect = self._make_effect()
        output = effect.backward(
            *self._effect_args_from_arrays(opens, highs, lows, closes, volumes)
        )
